...
- DESIGN-002 now has an isolated law kernel and strict V2 time-state for
  continuous measured salience, autonomous arousal evolution, bounded encoding
  gain, homeostatic scaling and rehearsal counts. The distributed medium
  remains on its V1 protocol until the versioned medium snapshot, private
  measurement path and replay whitelist are implemented together; this state
  is not wired as a hidden V1 compatibility patch.
