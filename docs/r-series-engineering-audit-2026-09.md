...
- DESIGN-002 now has an isolated law kernel for continuous measured salience,
  autonomous arousal evolution, bounded encoding gain and homeostatic scaling.
  The distributed medium remains on its V1 protocol until the versioned
  snapshot, private measurement path and replay whitelist are implemented
  together; the law kernel is not wired as a hidden V1 compatibility patch.
