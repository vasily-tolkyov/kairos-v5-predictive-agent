...
Current live work (2026-09-13): `peaceful-natural-online-long` retains the R2 model and starts from the untouched peaceful initial state; budget 2048 decisions / 3600 seconds, goal after 128 decisions, continuing learning after success. Its R3 actor remains immutable. A bounded R5 causal continuation uses the same existing world and exit goal after 945 decisions, with 128 new decisions / 420 seconds. These overlapping learning probes are not matched throughput comparisons. After the natural run stops, the registered [progress-recovery protocol](native-evidence-2026-09-13/peaceful-progress-recovery-protocol.md) compares frozen R3/R5 copies of its exact final state sequentially. No outcome is claimed before stopping and auditing.
