...
const [source, output, relocatedBuild] = process.argv.slice(2);
if (!source || !output) throw new Error('usage: NATIVE_MOTOR_CATCHUP_OUTPUT NEW_AUDIT_JSON [EXPLICIT_RELOCATED_BUILD]');
...
const build = resolve(relocatedBuild ?? report.build);
assert.equal(sha(await readFile(resolve(build, 'src/body.js'))), report.bodySha256);
...
const result = { version: 'NativeMotorReceiptAudit1', status: 'passed', source: root,
  recordedBuild: report.build, inspectedBuild: build,
